...
<body>
        ...
        <select id="reservationDate" name="reservationDate">
            ...
            <%
                try {
                    con = db.getConnection();
                    Statement stmt = con.createStatement();
                    ResultSet rs = stmt.executeQuery("SELECT DISTINCT DATE(DepartureDateTime) AS ReservationDate FROM Reservations ORDER BY ReservationDate");

                    while (rs.next()) {
            %>
                        <option value="<%= rs.getString("ReservationDate") %>"
                            <%= request.getParameter("reservationDate") != null && request.getParameter("reservationDate").equals(rs.getString("ReservationDate")) ? "selected" : "" %>>
    ...
    <table border="1">
        ...
        <%
            ...
            try {
                con = db.getConnection();
                String query = 
                    "SELECT r.ReservationID, c.FirstName, c.LastName, tl.LineName, s1.StationName AS OriginStation, " +
                    "s2.StationName AS DestinationStation, r.DepartureDateTime, r.TotalFare " +
                    "FROM Reservations r " +
                    "JOIN Customers c ON r.CustomerID = c.CustomerID " +
                    "LEFT JOIN TransitLines tl ON r.LineID = tl.LineID " +
                    "LEFT JOIN Stations s1 ON r.OriginStationID = s1.StationID " +
                    "LEFT JOIN Stations s2 ON r.DestinationStationID = s2.StationID " +
                    "WHERE 1=1 ";
                ...
                if ("originDestination".equals(searchMode)) {
                    if (originStation != null && !originStation.isEmpty()) query += "AND r.OriginStationID = ? ";
                    if (destinationStation != null && !destinationStation.isEmpty()) query += "AND r.DestinationStationID = ? ";
                }

                PreparedStatement pstmt = con.prepareStatement(query);

                int paramIndex = 1;
                if (reservationDate != null && !reservationDate.isEmpty()) pstmt.setString(paramIndex++, reservationDate);
    ...
    <!-- Back Button -->
    <div>
        <button type="button" onclick="window.location.href='CustomerRepresentative.jsp'">Back</button>
    </div>
</body>
</html>
