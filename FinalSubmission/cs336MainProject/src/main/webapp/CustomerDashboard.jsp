...
<body>
	...
	<form method="GET">
		...
		<!-- Travel Date -->
		<label for="travelDate">Select Date of Travel:</label> <select
			...
			try {
				...
				Statement stmt = con.createStatement();
				ResultSet rs = stmt
				.executeQuery("SELECT DISTINCT DATE(DepartureTime) AS TravelDate FROM TrainStops ORDER BY TravelDate");
				while (rs.next()) {
			%>
			<option value="<%=rs.getString("TravelDate")%>"
				<%=request.getParameter("travelDate") != null
		&& request.getParameter("travelDate").equals(rs.getString("TravelDate")) ? "selected" : ""%>>
			...
			%>
		</select> <label for="scheduleSort">Sort By:</label> <select id="scheduleSort"
			name="scheduleSort">
		...
		<button type="button" onclick="resetFilters()">View All
			Results</button>
	</form>
	<!-- Results Table -->
	<h2>Results:</h2>
	<table border="1">
		...
		try {
			con = db.getConnection();
			String query = "SELECT tl.LineName, s1.StationName AS OriginStation, s2.StationName AS DestinationStation, "
			...
			+ "JOIN Stations s2 ON ts_dest.StationID = s2.StationID " + "WHERE 1=1 ";
	...
	<form method="GET">
		<label for="searchKeyword">Search Questions:</label> <input
	...
	<table border="1">
		...
		try {
			con = db.getConnection();
			String query = "SELECT m1.Message AS Question, "
			...
			+ "c.ConversationID, c.Status " + "FROM Messages m1 "
			...
			<td>
				<form action="ViewConversation.jsp" method="GET"
					...
					<input type="hidden" name="conversationID"
						value="<%=rsQuestions.getInt("ConversationID")%>">
