...
<body>
	...
	try {
		...
		if (stopID != null) {
			...
			con.commit();
			out.println(
			"<h2>Schedule and all associated stops deleted successfully. <a href='CustomerRepresentative.jsp'>Go back</a></h2>");
		} else {
			out.println(
			"<h2>Invalid request. No schedule or stop specified. <a href='CustomerRepresentative.jsp'>Go back</a></h2>");
		}
	} catch (SQLException e) {
		con.rollback();
		out.println("<h2>Error processing request: " + e.getMessage()
		+ ". <a href='CustomerRepresentative.jsp'>Go back</a></h2>");
	} finally {
		try {
			con.setAutoCommit(true);
			con.close();
		} catch (SQLException e) {
			out.println("<h2>Error closing connection: " + e.getMessage()
			+ ". <a href='CustomerRepresentative.jsp'>Go back</a></h2>");
		}
	}
	%>
