...
<body>
	<h1>
		Edit Train Schedule for Schedule ID:
		<%=scheduleID%></h1>
	<form method="POST" action="UpdateSchedule.jsp">
		<table border="1">
			...
			<%
			try (PreparedStatement stmt = con.prepareStatement("SELECT ts.StopID, s.StationName, ts.ArrivalTime, ts.DepartureTime "
					+ "FROM TrainStops ts " + "JOIN Stations s ON ts.StationID = s.StationID "
			...
			<tr>
				...
				<td><input type="datetime-local"
					name="arrivalTime_<%=rs.getInt("StopID")%>"
					value="<%=rs.getTimestamp("ArrivalTime").toString().replace(" ", "T")%>">
				</td>
				<td><input type="datetime-local"
					name="departureTime_<%=rs.getInt("StopID")%>"
	...
	<form method="POST" action="AddStop.jsp">
		<input type="hidden" name="scheduleID" value="<%=scheduleID%>">
		<label for="station">Station:</label> <select name="stationID">
			<%
			...
			%>
		</select> <label for="arrivalTime">Arrival Time:</label> <input
			type="datetime-local" name="arrivalTime" required> <label
			for="departureTime">Departure Time:</label> <input
