...
<body>
	...
	<form action="AdminEmployeeManager.jsp" method="GET"
	...
	<form action="AdminRevenueList.jsp" method="GET"
		style="display: inline;">
	...
	String bestCustomerQuery = "SELECT CONCAT(c.FirstName, ' ', c.LastName) AS CustomerName, "
			+ "COUNT(r.ReservationID) AS NumberOfReservations " + "FROM Reservations r "
			+ "JOIN Customers c ON r.CustomerID = c.CustomerID " + "GROUP BY c.CustomerID, c.FirstName, c.LastName "
			+ "ORDER BY NumberOfReservations DESC " + "LIMIT 1";
	...
	<h3>Best Customer!</h3>
	<div
		style="padding: 10px; border: 2px solid #4CAF50; background-color: #e8f5e9; display: inline-block;">
		<h2
			style="color: #4CAF50; margin: 0; font-size: 1.5em; font-weight: bold;">
			<%=bestCustomerName%>
		</h2>
		<p style="margin: 0; font-size: 1.2em;">
			Reservations: <strong><%=bestCustomerReservations%></strong>
		</p>
	</div>
	...
	<table border="1">
		...
		String query = "SELECT tl.LineName AS TransitLine, " + "COUNT(r.ReservationID) AS NumberOfReservations "
				+ "FROM Reservations r " + "JOIN TransitLines tl ON r.LineID = tl.LineID " + "GROUP BY tl.LineName "
				+ "ORDER BY NumberOfReservations DESC " + "LIMIT 5";
