...
<body>
	...
	<form method="POST" action="SaveTrainSchedule.jsp">
		<!-- Select Train Line -->
		<label for="lineID">Select Train Line:</label> <select id="lineID"
			name="lineID" required>
			<%
		...
		<!-- First Stop -->
		<label for="firstStop">First Stop:</label> <select id="firstStop"
			name="firstStop" required>
			<%
		...
		<!-- Next Stop -->
		<label for="nextStop">Next Stop:</label> <select id="nextStop"
			name="nextStop" required>
			<%
			db = new ApplicationDB();
			con = db.getConnection();
		...
		<label for="arrivalTime">Arrival Time:</label> <input
			type="datetime-local" id="arrivalTime" name="arrivalTime" required>
		...
		<label for="departureTime">Departure Time:</label> <input
			...
			required> <span>(Example: 2024-12-04 07:10:00.0)</span><br>
		<br>
