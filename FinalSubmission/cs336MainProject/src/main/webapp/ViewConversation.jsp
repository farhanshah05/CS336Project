...
<body>
	...
	try {
		...
		"SELECT m.MessageID, m.Message, m.Timestamp, " + "CASE " + "WHEN m.SenderID = c.CustomerID THEN 'Customer' "
				+ "WHEN m.SenderID = e.EmployeeID THEN 'Representative' " + "ELSE 'Employee' END AS SenderType "
				+ "FROM Messages m " + "JOIN Conversations c ON m.ConversationID = c.ConversationID "
	...
	<h2>Reply</h2>
	<form method="POST">
		<textarea name="replyMessage" rows="4" cols="50" required></textarea>
		<br>
	...
	<br>
	<form
		action="<%="employee".equals(role) ? "CustomerRepresentative.jsp" : "CustomerDashboard.jsp"%>"
		method="GET">
		<button type="submit">
			Back to
			<%="employee".equals(role) ? "Dashboard" : "Customer Dashboard"%></button>
	</form>
	...
	try {
		...
		senderID = rs.getInt(1);
			...
			ps = con.prepareStatement("SELECT CustomerID FROM Conversations WHERE ConversationID = ?");
			ps.setInt(1, conversationID);
			rs = ps.executeQuery();
			if (rs.next()) {
		receiverID = rs.getInt(1);
		...
		out.println("<p>Message sent successfully. <a href='ViewConversation.jsp?conversationID=" + conversationID
		+ "'>Go back</a></p>");
	} catch (SQLException e) {
